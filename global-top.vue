...
const glossaryMap = {
  ...
  'Differential Privacy': { title: 'Differential Privacy', body: `dsFlower offers <strong>two flavours</strong> of differential privacy, depending on the framework and the guarantee you need.<br/><br/><strong>Formal DP-SGD</strong> (via Opacus): per-sample gradient clipping plus calibrated noise during training, giving a provable <strong>(ε, δ)</strong> bound. PyTorch only. The privacy budget is tracked per-dataset in a server-side ledger.<br/><br/><strong>Update-level noise</strong>: the weight delta is clipped and Gaussian noise added before it leaves the node. No formal guarantee, but works with <strong>any framework</strong> (sklearn, PyTorch, XGBoost) at a lighter accuracy cost.<br/><br/>Both share the same principle: the analyst can never trace the model back to a single patient.` },
  ...
  'Secure histogram': { title: 'Secure Histogram Aggregation (XGBoost)', body: `XGBoost builds <strong>decision trees</strong>, not neural networks. It cannot exchange weights like PyTorch or sklearn.<br/><br/>Each hospital computes <strong>gradient/Hessian histograms</strong> per feature. These are quantized to <strong>integers</strong> over a configurable range so SecAgg+ can mask them.<br/><br/>The SuperLink receives only the <strong>summed histograms</strong> and selects optimal tree splits. Individual hospital contributions are never visible.<br/><br/>The secure profile uses <strong>depth-1 boosted stumps</strong>, required by SecAgg+'s fixed-shape protocol. <strong>SecAgg+ is mandatory</strong> for XGBoost in dsFlower.` },
